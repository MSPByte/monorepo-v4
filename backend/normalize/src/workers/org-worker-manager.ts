...
import { createNormalizeWorker } from "./normalize-worker.js";
...
export function createOrgWorkerManager(redis: RedisConnection) {
  ...
  async function sync(): Promise<void> {
    ...
    for (const orgId of activeOrgIds) {
      ...
      const queueName = orgQueueName(QUEUES.NORMALIZE, orgId);
      const worker = createNormalizeWorker(redis, queueName);
      workers.set(orgId, worker);
      logger.info("Started normalize worker for organization", {
        orgId,
        queueName,
    ...
    for (const [orgId, worker] of workers) {
      ...
      await worker.close();
      logger.info("Stopped normalize worker for inactive organization", {
        orgId,
      });
    }
  }
