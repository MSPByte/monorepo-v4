...
import { getSophosRawSchema, PROVIDER_IDS, ProviderFacet } from '@mspbyte/shared';
import type {
  FetchPage,
  FetchResultCursor,
  IngestionAdapter,
  ...
  async *fetch(type, _mode, _cursor, context): AsyncGenerator<FetchPage, FetchResultCursor> {
    ...
    switch (facet) {
      ...
      case ProviderFacet.SophosFirewallLicenses:
        return yield* fetchFirewallLicenses(connector, context);
...
function createConnector(context: IngestionAdapterContext): SophosConnector {
  ...
  if (!clientSecret) {
    throw new Error(`Sophos client secret could not be decrypted for link ${context.linkId}`);
  }
...
function stringConfig(context: IngestionAdapterContext, key: string): string {
  ...
  if (typeof value !== 'string' || value.length === 0) {
    throw new Error(`Sophos integrationConfig.${key} missing for link ${context.linkId}`);
  }
  return value;
}
...
async function* fetchFirewalls(
  ...
  if (firewalls.length > 0 && tenantId) {
    try {
      ...
      const results = await connector.firewall.firmwareUpgradeCheck(apiHost, tenantId, ids);
      for (const fw of results) {
        if (fw.id) upgradeMap.set(fw.id, fw.upgradeToVersion?.[0] ?? null);
      }
    } catch (error) {
      logger.warn('Sophos firmware upgrade check failed, continuing without upgrade info', {
  ...
  const records = firewalls.map((fw) => ({
    ...fw,
    _upgrade_to_version: typeof fw.id === 'string' ? (upgradeMap.get(fw.id) ?? null) : null
  }));
...
async function* fetchFirewallLicenses(
  connector: SophosConnector,
  context: IngestionAdapterContext
): AsyncGenerator<FetchPage> {
  const tenantId = getTenantId(context);
...
async function* fetchTamperProtection(
  ...
  const tenantId = getTenantId(context);
  if (!tenantId) {
    throw new Error(`Sophos tenant externalId missing in linkMeta for link ${context.linkId}`);
  }
  if (!context.tenantDb) {
    throw new Error(
  ...
  for (const endpoint of rows) {
    try {
      ...
      batch.push({
        ...
        previousPasswords: normalizePreviousPasswords(tamperProtection.previousPasswords)
      });
