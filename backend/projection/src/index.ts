...
async function shutdown(signal: string) {
  logger.info("Shutting down projection service", { signal });
...
workerRefreshInterval = setInterval(() => {
  void workerManager.sync().catch((error) => {
    logger.error("Failed to sync projection workers", { error: serializeError(error) });
  });
...
logger.info("backend/projection has started", {
