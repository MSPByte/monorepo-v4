<script lang="ts">
  ...
  const filterOptions = $derived([{ id: 'all', label: 'All' }, ...filters]);
        ...
        <table class="w-full table-fixed text-sm">
          ...
          <tbody>
            {#each groups as entity (entity.entityKey)}
              ...
              {@const modules = [
                ...new Set(
                  entity.moduleIds.map((definitionId) => moduleLabelForDefinition(definitionId))
                ),
              ]}
