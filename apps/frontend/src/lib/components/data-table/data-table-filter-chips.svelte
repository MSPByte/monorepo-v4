<script lang="ts">
  ...
  let { filters, viewFilters = [], columns, onremovefilter, onclearfilters }: Props = $props();

  const viewFilterIds = $derived(new Set(viewFilters.map((f) => f.id)));

  // Only show dynamic filters (not from views)
  const dynamicFilters = $derived(filters.filter((f) => !viewFilterIds.has(f.id)));
  ...
  function getFilterLabel(filter: TableFilter): string {
    ...
    if (column?.filter?.type === 'date' && filter.value) {
      const rawDate = String(filter.value).slice(0, 10);
      const [year, month, day] = rawDate.split('-').map(Number);
      if (year && month && day) {
        valueLabel = new Date(year, month - 1, day).toLocaleDateString();
      }
    }
...
{#if dynamicFilters.length > 0}
  <div class="flex flex-wrap items-center gap-2">
    ...
    <Button variant="ghost" size="sm" onclick={onclearfilters} class="h-auto px-2 py-1 text-xs">
      Clear all
    </Button>
  </div>
{/if}
