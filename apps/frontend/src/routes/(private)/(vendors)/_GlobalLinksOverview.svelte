<script lang="ts" module>
  ...
  export type LinkOverviewExtraColumn = {
    key: string;
    label: string;
    widthClass?: string;
    align?: 'left' | 'center';
    searchable?: boolean;
    value?: (row: LinkOverviewRow) => unknown;
    format?: (value: unknown, row: LinkOverviewRow) => string;
    emptyWhenZero?: boolean;
    badgeClass?: (value: unknown, row: LinkOverviewRow) => string;
  };
...
<script lang="ts">
  ...
  let {
    ...
    extraColumns = [],
    showFindingSummary = true,
    showIndicatorColumn = true,
    showDispositionColumn = true,
    showNotesColumn = true,
    showFindingsColumn = true,
    showStatusColumn = true,
    onrowclick,
  }: {
    rows: LinkOverviewRow[];
    isLoading: boolean;
    isPending: boolean;
    vendorName: string;
    totalLabel?: string;
    ...
    extraColumns?: LinkOverviewExtraColumn[];
    showFindingSummary?: boolean;
    showIndicatorColumn?: boolean;
    showDispositionColumn?: boolean;
    showNotesColumn?: boolean;
    showFindingsColumn?: boolean;
    showStatusColumn?: boolean;
    onrowclick: (row: LinkOverviewRow) => void;
  } = $props();
  ...
  const criticalCount = $derived(
    filteredRows.filter((row) => (row.maxSeverity ?? -1) >= 3).length
  );

  const warningCount = $derived(
    filteredRows.filter(
      (row) => (row.maxSeverity ?? -1) >= 1 && (row.maxSeverity ?? -1) < 3
    ).length
  );

  const healthyCount = $derived(filteredRows.filter((row) => row.findingCount === 0).length);

  function searchValues(row: LinkOverviewRow) {
    return [
      row.siteName,
      row.linkName,
      row.externalId,
  ...
  function compareRows(a: LinkOverviewRow, b: LinkOverviewRow) {
  ...
  function statusBadge(maxSeverity?: number | null) {
    if (maxSeverity === 4) return { label: 'Critical', class: 'bg-destructive/15 text-destructive' };
    if (maxSeverity === 3) return { label: 'High', class: 'bg-destructive/10 text-destructive/80' };
    if (maxSeverity === 2) return { label: 'Medium', class: 'bg-warning/20 text-warning' };
    if (maxSeverity === 1) return { label: 'Low', class: 'bg-muted text-muted-foreground' };
    return { label: 'Healthy', class: 'bg-success/15 text-success' };
  }

  function statusDot(maxSeverity?: number | null) {
    if (maxSeverity === 4) return 'bg-destructive';
    if (maxSeverity === 3) return 'bg-destructive/80';
    if (maxSeverity === 2) return 'bg-warning';
    if (maxSeverity === 1) return 'bg-muted-foreground/40';
    return 'bg-success';
  }

  function columnValue(column: LinkOverviewExtraColumn, row: LinkOverviewRow) {
    return column.value?.(row) ?? row[column.key];
  }

  function hasColumnValue(column: LinkOverviewExtraColumn, value: unknown) {
  ...
  function formatColumnValue(
    column: LinkOverviewExtraColumn,
    value: unknown,
    row: LinkOverviewRow
  ) {
...
<FadeIn class="flex flex-col size-full overflow-hidden">
  <div
    class={cn(
      'grid gap-3 p-4 border-b shrink-0',
      showFindingSummary ? 'grid-cols-4' : 'grid-cols-2'
    )}
  >
    ...
    {#if showFindingSummary}
      <div class="rounded-lg border bg-card p-4 flex flex-col gap-1">
        ...
        <div class="text-xs text-muted-foreground">highest finding severity</div>
      </div>
      <div class="rounded-lg border bg-card p-4 flex flex-col gap-1">
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
          Low/Medium
      ...
      <div class="rounded-lg border bg-card p-4 flex flex-col gap-1">
        ...
        <div class="text-xs text-muted-foreground">no open findings</div>
      </div>
    {/if}
  </div>
        ...
        <table class="h-fit w-full text-sm">
            ...
            <tr class="border-b text-xs text-muted-foreground uppercase tracking-wide">
              ...
              {#if showFindingsColumn}
                <th class="px-4 py-2 text-center w-24">
                  ...
                  >
                    Findings
                    {@render sortIndicator('findingCount')}
                  </button>
                </th>
              {/if}
              {#if showStatusColumn}
                <th class="px-4 py-2 text-center w-28">
                  <button
                    type="button"
                    class="inline-flex items-center justify-center gap-1.5 hover:text-foreground"
                    onclick={() => toggleSort('maxSeverity')}
                  >
                    Status
                    {@render sortIndicator('maxSeverity')}
                  </button>
                </th>
              {/if}
            </tr>
          ...
          <tbody>
            {#each filteredRows as row (row.linkId)}
              {@const status = statusBadge(row.maxSeverity)}
